...
class SimpleFilterMain: BaseViewController {
    ...
    let AUDIO_FILTER_NAME = "OfflineVoiceConversion"
    
    @IBOutlet weak var segment: UISegmentedControl!
    
    @IBOutlet weak var voiceNameSegment: UISegmentedControl!
    ...
    override func viewDidLoad(){
       ...
       // TODO: 测试的 clientId和ClientSecret，需要进行替换
        let index = UserDefaults.standard.integer(forKey: "index")
        guard index < segment.numberOfSegments else {
            print("[error]: index beyond the segments")
            return
        }
        segment.selectedSegmentIndex = index
        
        let voiceName = voiceNameWithSegemnt(index)
//        let clientId = "xxx"
//        let clientSecret = "xxx"
      
        let authorInfo = [
            "clientId":KeyCenter.clientId,
            "clientSecret":KeyCenter.clientSecret,
            "voiceName":voiceName
        ]
...
//        print("--- set the authors")
        ...
        agoraKit.setExtensionPropertyWithVendor(DBVoiceConvertFilterManager.vendorName(), extension: AUDIO_FILTER_NAME, key: "isLog", value: String(false))
    }
    
    
    @IBAction func SelectSegmentControl(_ sender: UISegmentedControl) {
        let index = sender.selectedSegmentIndex;
        let value = voiceNameWithSegemnt(index)
    ...
    func voiceNameWithSegemnt(_ index:Int) ->String {
    
        let voiceArray = ["Vc_luoli","Vc_dashu","Vc_kongling","Vc_bawanglong","Vc_zhongjinshu"]
        guard index < voiceArray.count else {
            print("out of the array bound");
            return ""
        }
        let voiceName = voiceArray[index]
        return voiceName
    }
    ...
    @IBAction func onOpen(_ sender: UISwitch) {
        agoraKit.setExtensionPropertyWithVendor(DBVoiceConvertFilterManager.vendorName(), extension: AUDIO_FILTER_NAME, key: "enable", value: String(sender.isOn))
    }
    
    @IBAction func stopPreview(_ sender: UIButton) {
...
// 需要验证的三个问题： 1. setproperty的调用； 2.获取datasource的回调； 3.音频处理的同步处理方法；
